...
def launch_flame(dcc_path, dcc_args):
    ...
    flame_engine = sgtk.platform.start_engine(
        engine_instance_name, context.sgtk, context
    )
    ...
    if not python_executable:
        flame_engine.log_error(
            "Cannot find environment variable TOOLKIT_FLAME_PYTHON_BINARY"
        )
    else:
    ...
    if not install_root:
        flame_engine.log_error(
            "Cannot find environment variable TOOLKIT_FLAME_INSTALL_ROOT"
        )
    else:
    ...
    if None in (
        ...
        flame_engine.log_error(
            "Cannot find environment variable TOOLKIT_FLAME_x_VERSION"
        )
    else:
        flame_engine.set_version_info(
