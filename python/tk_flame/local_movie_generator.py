...
class LocalMovieGenerator(object):
    ...
    def generate(
        ...
        it to a list of Flow Production Tracking entities in the Path to movie field.

        :param src_path: Path to the media for which a local movie need to be
            generated and linked to Flow Production Tracking.
        ...
        self.engine.create_local_backburner_job(
            job_name="%s - Updating Flow Production Tracking Path to movie"
            % display_name,
            description="Uploading Flow Production Tracking Path to movie to %s"
            % dst_path,
            dependencies=job_id,
